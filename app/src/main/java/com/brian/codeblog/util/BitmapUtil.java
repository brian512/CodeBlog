...
public class BitmapUtil {
    ...
    public static Bitmap getReflectedImage(Bitmap originalImage, float percent, int reflectionGap) {
        int height = originalImage.getHeight();
        int refHeight = (int) (height * percent);
        return getReflectedImage(originalImage, refHeight);
    }
    ...
    public static Bitmap getReflectedImage(Bitmap originalImage, int refHeight, final int reflectionGap) {
        ...
        Bitmap reflectionImage = Bitmap.createBitmap(originalImage, 0, height
                - refHeight, width, refHeight, matrix, false);
        Bitmap reflection = Bitmap.createBitmap(width, refHeight,
                Config.ARGB_8888);
    ...
    public static boolean createReflectedImage(String srcPath, String refPath,
            float percent, int reflectionGap) {
        if (TextUtils.isEmpty(srcPath) || TextUtils.isEmpty(refPath)) {
            return false;
        }
        if (ImageUtils.checkImageFileExist(refPath)) {
            return true;
        }
        Params params = ImageUtils.getImageParams(srcPath);
        if (params == null) {
            LogUtil.v("params=null");
            return false;
        }
        return createReflectedImage(srcPath, refPath,
                (int) (params.height * percent), reflectionGap);
    }

    public static boolean createReflectedImage(Bitmap srcBp, String refPath,
            float percent, int reflectionGap) {
        LogUtil.v("refPath=" + refPath);
        if (srcBp == null || TextUtils.isEmpty(refPath)) {
            return false;
        }
        if (ImageUtils.checkImageFileExist(refPath)) {
            return true;
        }
        Params params = ImageUtils.getImageParams(srcBp);
        if (params == null) {
            return false;
        }
        return createReflectedImage(srcBp, refPath, (int) params.height,
                reflectionGap);
    }

    public static boolean createReflectedImage(String srcPath, String refPath,
            int refHeight, int reflectionGap) {
        if (TextUtils.isEmpty(srcPath) || TextUtils.isEmpty(refPath)) {
            return false;
        }

        if (ImageUtils.checkImageFileExist(refPath)) {
            changeLastModified(refPath);
            return true;
        }

        Bitmap srcBp = BitmapFactory.decodeFile(srcPath);
        return createReflectedImage(srcBp, refPath, refHeight, reflectionGap);
    }
    ...
    public static boolean createReflectedImage(Bitmap srcBp, String refPath,
        ...
        if (ImageUtils.checkImageFileExist(refPath)) {
            LogUtil.v("image exist : " + refPath);
            changeLastModified(refPath);
            srcBp.recycle();
            return true;
        }
        Bitmap refBp = getReflectedImage(srcBp, refHeight, reflectionGap);
        if (srcBp != null && !srcBp.isRecycled()) {
            srcBp.recycle();
        }
        if (refBp == null) {
            LogUtil.e("refBp = null");
            return false;
        }
        boolean result = FileUtil.writeBitmap(refPath, refBp);
        refBp.recycle();
